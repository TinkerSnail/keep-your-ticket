...
| `ground_contact_test.gd` | Does every moving guest have real collision immediately underfoot, and every seated guest a seat at `seat_height`? | Reports every failure, one line per surface hit, with its count, gap range, extent and crowds. Until 2026-09-25 it printed only the first 80 sorted messages, all one guest's, which is why the standing failures read as "only the east-wing ramps"; `prop_handback.py`'s `KNOWN_GROUND` lists them now. |
...
| `day_test.gd` | Does each section's crowd have a day — the curves, the admitting and the sending home? | `--headless --fixed-fps 60`; about five minutes a section in the persistent world (2026-09-25), so allow twenty. |
...
| `_crowd_graph_section_probe.gd` | Along every crowd edge, does the height a guest is given (the line between its nodes) match the built floor, on the centreline and 0.6 m either side? Prints only the edges that miss, and what they meet. Separates a wrong graph from guests straying off a right one. Headless, through `run.tscn`. |
