...
| `tunnel_drive_test.gd` | Can a car drive the coast highway's five tunnels — every bore, both directions — without meeting terrain in the road, and is there headroom over each carriageway? | Standing baseline **18 blocked segments**, every mouth of every bore, 2026-09-22. Rays each station to the next at 1.5m: direction matters, since a ray stops on ground rising ahead of it and one direction alone finds only the entries. Nothing else in the set drives a bore — `path_ground_test` rays downward, which a tunnel passes by construction. |
