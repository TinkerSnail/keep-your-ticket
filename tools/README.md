...
| `_palm_placement_migration.gd` | One-off, run 2026-09-25: turned what `coastal_palm_replacements.gd` built at the 34 generated palm feet into editor-owned `coastal_palm.tscn` placements in `approach_palms.tscn` and `boardwalk_palms.tscn`, and checked each trunk, crown seat and bed landed within 0.1 mm. The wrappers no longer mount the controller, so it cannot run again as is. Through `run.tscn`. |
