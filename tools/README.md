...
| `blender/paint_canvas.py` | A prop's paint canvas and its one material, on the game-ready working file: bakes each material's colour into `assets/source/textures/<prop>/<prop>_colour.png` (default 2048) and its roughness and metalness into `<prop>_orm.png` (default 1024, occlusion/roughness/metal in R/G/B), draws `<prop>_uv_guide.png` (piece outlines, transparent), then replaces the prop's materials with one that reads both. Refuses to overwrite a colour or ORM PNG; `--guide-only` redraws the guide. `-- [--size N] [--orm-size N] [--guide-only] [--save]`. |
