...
**Open, in the order it seems to matter.** Finish the park-section map-to-Godot completion gate before resuming the Kiddieland story slice: publish and prove the vertical drainage and Kiddieland source work, then promote the other eight section layouts to editor-owned sources. Asking a guest for a copy issues a ticket number and prints nothing. No guest has an errand; the schedule is the crowd's, not the individual's. Nobody queues, and nobody treats the pier as a destination. Rebuild packages 06 to 09 are planned and not built. The world beyond the park has no cars, no far headland and no sea stacks.
