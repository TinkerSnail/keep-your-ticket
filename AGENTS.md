...
**Generated sources under editor wrappers.** `tools/gen_props.gd` writes only `scenes/world/generated/*.tscn`. The same-named file one level up, `scenes/world/plaza_frontage.tscn` and so on, is a stable inherited wrapper and is the file to open in Godot: inspector changes become overrides there, and new hand-made objects go under its `authored_additions` node. Since 2026-09-19 a mesh or trimesh collision shape over 64KB is written beside its scene as binary, `generated/<scene>_data/*.res`, because 78MB of text terrain cost 12 seconds at every launch; the two protected anchors are exempt and stay one text file each. Compare such a scene with `tools/_generated_compare_probe.gd`, not the text filter. Runtime code never references `generated/`. `plaza.tscn` and `park_world.tscn` are hand-authored; the three crowd scenes are pure output of `tools/gen_crowd.gd`. A generator run that changes a wrapper is a regression.

**What stands.** The 104m plaza with its fountain at the origin, 40m clock tower, bandstand, photo hut, perimeter frontage on both faces and seven ways out; the entrance street, arcade, turnstiles, apron, parking fields and approach road; the west cascade, arrival court, boardwalk, pier, wheel and wooden coaster; the east gate, forecourt, east cascade, belvedere, basin staircase, hill and shoulders; the Park Promenade; the Grove, Kiddieland, Fairground and Frontier as greybox rooms on real routes; routes A to F and junctions J1 to J9 of the rebuild atlas, with its ride parcels, midway bands, recurring interiors, service spines and Coastal Creek as greybox; the Grand Circuit road train and the Kiddieland railway; the sky ride and the observation tower; night lighting on a schedule. Outside the program envelope: the generator-owned crescent range remains the primary continuous world geography. Its former suppressed summit is restored. The mounted editor-owned range source retains only the forest-green near layers plus one broad closed southern massif and one broad closed eastern connection; the failed five-piece blue perimeter shell and detached bald duplicate are retired. The coast remains a bay with a divided highway in its own cut corridor, twin-bore tunnels and the park's diamond interchange; the headland, lighthouse, harbour and two towns remain. A separate closed Morro-like volcanic plug now meets the north shoreline beyond the north town, clear of the town and highway. The former far-city skyline is retired while its land serves as the highway approach. A directly editable hooked peninsula carries the relocated city, a 410m vermilion suspension bridge, two ferry landings and one bay ferry. Broad closed foothills connect the city to the southern massif, with a beach skirt, city bypass and terrain-cut Pacific-Coast-Highway-style branch; both road profiles are capped at 1.9% grade.

The bypass and coastal branch stand on closed collision earthworks; mounted centreline-to-ground gaps are no greater than 0.25m.

**Systems.** `ParkClock` runs the day at twelve times real, open ten to ten. `daylight.gd` drives sun, sky and ambient from real solar geometry for latitude 32.75; since 2026-09-18 distance is a camera-distance depth haze on the Environment in `scenes/main/main.tscn`, tuned in the inspector and scaled down after dark by `daylight.gd`'s `HAZE_STRENGTH`; nothing is painted blue for distance, every surface wears its local colour, and both player cameras see to 4200m. The crowd (`crowd.gd`, `guest.gd`) has per-area headcount curves, groups admitted and sent home through the gate, an attention stack, contagious noticing, posing on request, sitting, wheelchairs and buggies, and step-free routing. `ParkGuests` holds a roster with claim-ticket debts and nothing binds it yet. `PhotoAlbum` writes PNGs to `user://photos`. The HUD owns the capture; the viewfinder, album, pause menu and map are in `scenes/ui/`. `park_lights.gd` and `ride_vehicle.gd` run the lights and trains. `range_forest_background.tscn` is the stable mounted range slot and now instantiates the directly editable `assets/source/range_forest_distinct_background.blend` review source. Both Blender world sources, the range and `far_shore_city_relocation.blend`, hold visible objects only. `tools/export_world_source.sh <master.blend>` writes the mounted GLB: every mesh under its own name with the `-col` import hint so Godot derives its collision, boulders baked into one mesh per 250m plan cell, and `kyt_collision=none` on anything that must not collide. Run it after any edit to a master, then `Godot --headless --path . --import`; the master is never saved by the export. The rejected attached ridge crowns, older fitted single ridge, full-depth sheets and runtime forest sources remain beside it for reversible comparison and recovery.
...
| `scenes/world/` | The wrappers, `plaza.tscn`, `park_world.tscn`, `daylight.gd`, `tower_clock.gd`, `park_lights.gd`, and the mounted distinct-background range slot `range_forest_background.tscn`; `range_forest_canopy.tscn` and `range_forest.gd` are unmounted recovery sources. |
...
**Acceptance set**, run before any commit that touches geometry, routes or crowds: `coplanar_test.py`, `footprint_test`, `footprint_walk_test`, `path_ground_test`, `clearance_test`, `anchor_walk_test`, `boardwalk_edge_test`, `section_test`, `transit_test`, `night_test`, `climb_test`, `budget_test`, `tunnel_drive_test`. The measurements and narrower checks (`perf_test`, `day_test`, `inpool_test`, `seat_test.py`, `ground_contact_test`, `menu_test`) run when their subject changes. `budget_test` also runs whenever a scene, prop or plant is added or a builder changes: it holds every view, the world and every repeated placement to a ceiling on surfaces and triangles, and prints what each placed scene costs. Raising a ceiling is Christina's decision. Three of the set fail at recorded baselines rather than clean: `coplanar_test.py` at 129 pairs, `footprint_test` at 98 open faces, and `path_ground_test` at one bury, `highway_b` 1.80m inside the corridor at (352.2, 133.2), all as of 2026-09-22. Compare against those numbers, not against zero. The legacy `walk_test` was split into the two anchor and edge tests and retired on 2026-09-05.
...
- Assume the mounted geography is open world: any place may eventually be walked, driven through or flown over, and there is no offstage side. Treat the world spatially, never as a set of proof cameras. Camera labels are diagnostic and do not establish geography. A background object is beyond the complete built footprint in world coordinates, with plan-view exclusion from park, coast, town, road, bridge and city geometry; it must also survive arbitrary walk, drive and flyover views. Occlusion or current access limits do not excuse physical overlap.
- Cameras and saved views belong to Christina. Do not create, move, edit or use them as acceptance proof unless she explicitly asks. Prove world geometry from its mounted sources, plan footprint, topology, cross-sections, collision and traversable connections; Christina uses her cameras for visual review.
